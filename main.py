    ...
    try:
        with open(ruta_archivo, 'r', encoding='utf-8') as f:
            for linea in f:
                ...
                # Separar por comas según tus datos
                partes = linea.split(',') 
                if len(partes) >= 3:
                    tareas.append({
                        'id': partes[0].strip(),
...
# 3. EL ALGORITMO (planificación segun el diagrama)
def planificar_ep(tareas, recursos):
    # Iniciar la disponibilidad de recursos en tiempo = 0
    tiempo_recursos = {r_id: 0 for r_id in recursos}
    cronograma = []

    # Ordenar las tareas de mayor a menor segun su duración
    ...
    for tarea in tareas_ordenadas:
        ...
        # Filtrar los recursos compatibles con la categoría de cada tarea
        ...
        # Seleccionar el recurso compatible que se libere antes
...
def main():
    ...
    # 4. Calculo del makespan obtenido para mostrarlo en tu terminal
    if cronograma:
